...
class NeuralNet:
    ...
    def leaky_relu(self, alpha, u):    #to do QS 2 
        """
        ...
        """
        leaky = np.where(u > 0, u, alpha * u)
        return leaky 


        #raise NotImplementedError

    def derivative_leaky_relu(self, alpha, u):   #to do qs 2 
        """
        ...
        """
        d_leaky =  np.where(u > 0, 1, alpha)
        return d_leaky


    def softmax(self, u):      #Qs1 Softmax in ipynb, DONE passed 
        """
        ...
        u -= u_max[:, np.newaxis]
        exp_u = np.exp(u)
        sum_exp_u = np.sum(exp_u, axis=1)
        o = exp_u / sum_exp_u[:, np.newaxis]
        return o 
    ...
    def _dropout(u, prob):
        ...
        """
        rand_drop = np.random.choice([0, 1], size=u.shape, p=[prob, 1 - prob])
        post_drop = u * rand_drop
        post_drop /= (1 - prob)  

        return post_drop, rand_drop
    ...
    def cross_entropy_loss(self, y, y_hat):
        ...
        c = 1e-15
        y_hat +=  1e-15
        return -np.sum(y * np.log(y_hat + c)) / y.shape[0]
    ...
    def forward(self, x, use_dropout):
        ...
        u1 = np.dot(x, self.parameters["theta1"]) + self.parameters["b1"]  # TODO
        ...
        u2 = np.dot(o1, self.parameters["theta2"]) + self.parameters["b2"]  # TODO
    ...
    """
        if use_momentum: 
            for layer, dW in dLoss.items():
                if layer not in self.change: 
                    self.change[layer] = np.zeros_like(dW)
                self.change[layer] = self.momentum * self.change[layer] - self.learning_rate * dW
                self.parameters[layer] += self.change[layer]
        else: 
            # for layer, dW in dLoss.items():
            #     self.parameters[layer] -= self.learning_rate * dW
            if "thetha1" in dLoss: 
                self.parameters["theta1"] -= 0.0001 * self.learning_rate * dLoss["theta1"]
            for layer, dW in dLoss.items(): 
                if layer != "thetha1": 
                    self.parameters[layer] -= self.learning_rate * dW
    ...
    def compute_gradients(self, y, yh, use_dropout):
        ...
        dLoss_u2 = yh - y
        dLoss_theta2 = np.dot(self.cache["o1"].T, dLoss_u2) / self.sample_count
        dLoss_b2 = np.sum(dLoss_u2, axis=0) / self.sample_count
        dLoss_o1 = np.dot(dLoss_u2, self.parameters["theta2"].T)

        if use_dropout:
            dLoss_u1 = dLoss_u1 = dLoss_o1 * self.derivative_leaky_relu(self.alpha, self.cache["u1"]) * self.cache["mask"] / (1 - self.dropout_prob)
        else:
            dLoss_u1 = dLoss_o1 * self.derivative_leaky_relu(self.alpha, self.cache["u1"])


        dLoss_theta1 = np.dot(self.cache["X"].T, dLoss_u1) / self.sample_count
        dLoss_b1 = np.sum(dLoss_u1, axis=0) / self.sample_count
    ...
    def backward(self, y, yh, use_dropout, use_momentum):
        ...
        """
        # TODO
        gr = self.compute_gradients(y, yh, use_dropout)
        self.update_weights(self.compute_gradients(y, yh, use_dropout), use_momentum)
        dLoss_theta2 = gr["theta2"]
        dLoss_b2 = gr["b2"]
        dLoss_theta1 = gr["theta1"]
        dLoss_b1 = gr["b1"]
    ...
    def batch_gradient_descent(self, x, y, use_momentum, iter=60000, local_test=False):
        ...
        for i in range(iter):
            end_index = (start_index + batch_size) % x.shape[0]
            if end_index < start_index:
                indices = np.concatenate((np.arange(start_index, x.shape[0]), np.arange(0, end_index)))
            else:
            ...
            self.backward(y_batch, yh, self.use_dropout, use_momentum)
