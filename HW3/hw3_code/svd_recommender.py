...
class SVDRecommender(object):
    ...
    def recommender_svd(self, R: np.ndarray, k: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        ic = ImgCompression()
        U, S, V = ic.svd(R)
        U_k, S_k, V_k = ic.compress(U, S, V, k)
        #to return UK- U_k*S_k^^2   VK- Sk sq reshape * vk 
        skss = np.sqrt(S_k)
        return U_k*skss, skss.reshape(-1, 1)*V_k
    ...
    def predict(
        ...
        """
        index = users_index[user_id] ##change 
        notwatchedmov = np.argwhere(np.isnan(R[index, :]))
        rf=np.matmul(U_k,V_k)
        topmovies={}
        for m in movies_pool:
            if movies_index[self.get_movie_id_by_name(m)] in notwatchedmov:
                topmovies[m]=rf[index,movies_index[self.get_movie_id_by_name(m)]]
        recos_movies = np.array(sorted(topmovies, key=topmovies.get, reverse=True)[:top_n])
        return recos_movies
