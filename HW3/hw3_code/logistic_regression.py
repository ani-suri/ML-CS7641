...
class LogisticRegression(object):
    ...
    def predict_probs(
        ...
        """
        mult = np.matmul(x_aug, theta)
        h_x = self.sigmoid(mult)
        return h_x
    ...
    def predict_labels(self, h_x: np.ndarray) -> np.ndarray:  # [2pts]
        ...
        """
        hxflat = h_x.flatten()
        predictions = np.zeros(h_x.shape[0])
        predictions[hxflat > 0.5] = 1
    ...
    def loss(self, y: np.ndarray, h_x: np.ndarray) -> float:  # [3pts]
        ...
        Return:
            loss (float)
        """
        res = np.dot(y.T,np.log(h_x)) + np.dot(1-y.T, np.log(1 - h_x))
        loss = - (1 / y.shape[0] ) * res
        return loss[0,0]
    ...
    def evaluate(
        ...
        probs = self.predict_probs(xAug,theta)
        lab= self.predict_labels(probs)
        loss = self.loss(y,probs)
        accuracy = self.accuracy(y,lab)
    ...
    def fit(
        ...
        ### START STUDENT CODE ###
        ##########################
        aug = self.bias_augment(x_train)
        theta = np.zeros(((x_train.shape[1])+1,1))


        for i in range(epochs):
            h_x = self.predict_probs(aug, theta)
            grad = self.gradient(aug, y_train, h_x)
            theta -= lr*grad
            if i%100 == 0:
